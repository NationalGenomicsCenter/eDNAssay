...
aps <- read.csv(file.choose(), header = TRUE) # Specificity file output by eDNAssay
aps <- data.table(aps)

aps[, ':='(Min_amp = min(Amp), Max_amp = max(Amp), Mean_amp = mean(Amp), SD_amp = sd(Amp), N = length(Amp)), by = Taxon]
aps[is.na(aps)] <- 0
aps <- aps[order(Taxon), ]
